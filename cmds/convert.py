				...
				try:
					async with aiohttp.ClientSession() as s:
						async with s.get('http://free.currencyconverterapi.com/api/v3/convert?q={}_{}&compact=y'.format(
							...
							msg = await dclient.send_message(channel, '`{}: {:,.2f}` > `{}: {:,.2f}`'.format(cc, b, ct, nb))
                    ...
                    return True, 'HTTP', 'Error when trying to retrieve data from http://free.currencyconverterapi.com/api/v3/convert. ERROR: {}'.format(e.args[1]), msg
            else:
                msg = await dclient.send_message(channel, '{} Invalid currency code! Please check https://currencysystem.com/'
                                                    'codes/!'.format(mention))
        else:
            msg = await dclient.send_message(channel, '{}, currency must be in numeric/decimal value (ex: `100` or `54.42`) '
                                                'or must be in #type format (ex: `1m` = 1 million or `2k` = 2 thousand '
                                                'or `5b` = 5 billion)!'.format(mention))
    else:
        msg = await dclient.send_message(channel, '{}, **USAGE:** {}convert <amount> <from-currency-code> <to-currency-code>'
