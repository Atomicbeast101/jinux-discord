...
async def ex(dclient, channel, mention, a, cmd_char):
    ...
    if len(ar) >= 2:
        ...
        if lang_id.upper() in LANG_LIST:
            ...
            msg = await dclient.send_message(channel, 'Invalid language code input, `{}`! Please check <https://www.'
                                                'sitepoint.com/web-foundations/iso-2-letter-language-codes/> '
                                                'for correct language code, {}!'.format(lang_id, mention))
    else:
        msg = await dclient.send_message(channel, '{}, **USAGE:** {}trans <language-code> <to-translate>'.format(mention,
    return msg                                                                                            cmd_char))
