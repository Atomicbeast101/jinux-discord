...
async def ex(dclient, channel, mention, a, cmd_char):
    ...
    if len(a) > 0:
        ...
        try:
            ...
            if not v:
                msg = await dclient.send_message(channel, 'Unable to find any results, {}!'.format(mention))
            else:
                i, f = 0, False
                while not f and i < 20:
                    ...
                    i += 1
                if not f:
                    msg = await dclient.send_message(channel, 'Unable to find the link in the results, {}!'.format(mention))
                else:
            ...
            return True, 'HTTP', 'Error when trying to retrieve data from https://www.youtube.com/results. ERROR: {}'.format(e.args[1]), msg
    else:
        msg = await dclient.send_message(channel, '{}, **USAGE:** {}youtube <to-search>'.format(mention, cmd_char))
